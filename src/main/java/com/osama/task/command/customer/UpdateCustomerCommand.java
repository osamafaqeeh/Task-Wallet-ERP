 ...
 * @see CustomerService
 */
public class UpdateCustomerCommand implements ActionCommand {

    private static final Logger LOGGER = Logger.getLogger(UpdateCustomerCommand.class.getName());

    /**
     * Implementation of command to update customer information.
     *
    ...
    public Page execute(HttpServletRequest request) {
        ...
        String activeCustomerValue = request.getParameter(ACTIVE_CUSTOMER_PARAMETER);
            ...
            boolean isOperationSuccessful =  customerService.updateCustomer(customerIdValue, name, country, city, address, taxNumber, activeCustomerValue);
            if (!isOperationSuccessful) {
                return new Page(INFORMATION_FORM_PAGE_PATH, false, INVALID_INPUT_DATA_MESSAGE_KEY);
            }

            //update the customer list after update customer.
